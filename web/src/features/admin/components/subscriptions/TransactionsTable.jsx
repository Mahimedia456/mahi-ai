      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          <thead className="border-b border-white/5 bg-[#171717]">
            <tr>
              {["Invoice", "User", "Plan", "Amount", "Method", "Status", "Date"].map((head) => (
                <th
                  key={head}
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.id} className="border-b border-white/5 last:border-b-0">
                <td className="px-6 py-5 text-sm font-semibold text-white">{item.invoice}</td>
                ...
                <td className="px-6 py-5 text-sm text-[#d2dbd9]">{item.method}</td>
                <td className="px-6 py-5">
